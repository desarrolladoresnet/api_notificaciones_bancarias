...
type getNotificationsBancaribe struct {
	Referencia      string `form:"referencia"`
	Fecha           string `form:"fecha"`
	TelefonoCliente string `form:"telefono_cliente"`
...
func GetPayments(db *gorm.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		var request getNotificationsBancaribe
...
func validateRequestParams(request getNotificationsBancaribe) (bool, []string) {
	var errors []string
	...
	if request.TelefonoCliente != "" {
		if !isValidVenezuelanPhone(request.TelefonoCliente) {
