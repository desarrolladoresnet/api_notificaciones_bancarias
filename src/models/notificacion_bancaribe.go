...
type NotificationBancaribe struct {
	...
	CommercePhone        string    `gorm:"size:16;column:commerce_phone"`
	CreditorAccount      string    `gorm:"size:50;column:creditor_account"`
	CurrencyCode         string    `gorm:"size:5;column:currency_code"`
	DateBancaribe        string    `gorm:"size:12;column:date_bancaribe"`
	...
	OriginBankReference  string    `gorm:"size:15;column:origin_bank_reference"` // Corregido "colum:" a "column:"
	PaymentType          string    `gorm:"size:6;column:payment_type"`
	TimeBancaribe        string    `gorm:"size:10;column:time_bancaribe"`
	Time                 time.Time `gorm:"type:time;column:time"`
}
